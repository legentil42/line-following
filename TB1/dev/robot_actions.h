...
class Robot_actions_c {
  public:
    ...
    void turn_theta_degrees(float desired_degrees) {
        float desired_rad = desired_degrees * 2 * PI /360;
        Kin.update();
        reset_count();
        float old_theta = Kin.theta;
        while (abs(desired_rad-(Kin.theta-old_theta)) > 0.05*desired_rad) {
          Kin.update();
          Serial.print(desired_rad-Kin.theta);
          Serial.print(" ,");
          Serial.println("going");
          Motors.L_speedo = 10+(0.98*desired_rad-(Kin.theta-old_theta))*10+ (abs(count_e0)-abs(count_e1))*1;
          Motors.R_speedo = -10-(0.98*desired_rad-(Kin.theta-old_theta))*10- (abs(count_e1)-abs(count_e0))* 1;
